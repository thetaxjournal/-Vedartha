...
import { Receipt, PlusCircle, CheckCircle2, Clock, Printer, X, Download, Eye, Search } from 'lucide-react';
import QRCode from 'react-qr-code';
import { Invoice, Payment, Branch } from '../types';
import { COMPANY_LOGO, COMPANY_NAME, INITIAL_BRANCHES } from '../constants';
...
const Payments: React.FC<PaymentsProps> = ({ invoices, payments, onRecordPayment }) => {
  ...
  const getBranchForPayment = (payment: Payment): Branch => {
    ...
    return INITIAL_BRANCHES.find(b => b.id === inv?.branchId) || INITIAL_BRANCHES[0];
  };
  ...
  const ReceiptDocument = ({ payment }: { payment: Payment }) => {
    ...
    const qrValue = `VED-RCPT|SIGN:${signatureHash}|ID:${payment.id}|AMT:${payment.amount}|CLI:${payment.clientName.slice(0,15)}`;
          ...
          <table className="w-full text-[11px] border-collapse text-[#000000]">
            ...
            <tbody className="divide-y border-b-2 border-[#000000]">
              <tr className="align-top">
                ...
                <td className="py-6 text-right font-bold text-[14px]">₹ {payment.amount.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
              </tr>
            </tbody>
          </table>
        </div>
              ...
              <tbody className="divide-y divide-gray-50">
                {filteredPayments.map(pay => (
                  <tr key={pay.id} className="group hover:bg-gray-50 transition-colors">
                    ...
                    <td className="py-5 text-right font-bold text-emerald-600">₹ {pay.amount.toLocaleString('en-IN')}</td>
