...
const Dashboard: React.FC<DashboardProps> = ({ invoices, clients, branches, payments }) => {
  const mtdRevenue = invoices.reduce((acc, inv) => acc + inv.grandTotal, 0);
  const receivableCount = invoices.filter(inv => inv.status === 'Posted').length;
  const receivableValue = invoices.reduce((acc, inv) => acc + inv.grandTotal, 0);
  ...
  const pieData = branches.map(b => ({
    ...
    value: invoices.filter(inv => inv.branchId === b.id).reduce((acc, inv) => acc + inv.grandTotal, 0) || 100
  }));
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex justify-between items-end mb-10 pb-6 border-b border-gray-200">
        ...
        <div className="flex space-x-3">
          <div className="bg-white px-5 py-2.5 rounded-2xl border border-gray-200 shadow-sm flex items-center space-x-3">
            ...
            <span className="text-[11px] font-black text-gray-500 uppercase tracking-wider">Cloud Engine Active</span>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: 'Revenue (MTD)', value: `₹ ${mtdRevenue.toLocaleString('en-IN')}`, trend: '+12.5%', icon: TrendingUp, color: 'blue', sub: 'vs Previous Period' },
            ...
            {pieData.map((item, idx) => (
              <div key={idx} className="flex justify-between items-center group cursor-pointer hover:bg-gray-50 p-3 rounded-2xl transition-all border border-transparent hover:border-gray-100">
                ...
                <div className="flex flex-col items-end">
                   <span className="text-[11px] font-black text-[#003366]">₹ {item.value.toLocaleString('en-IN')}</span>
      ...
      <div className="bg-gradient-to-r from-[#003366] to-[#0854a0] p-6 rounded-[32px] shadow-2xl shadow-blue-200 flex items-center justify-between text-white overflow-hidden relative">
        ...
        <div className="flex items-center space-x-6 relative z-10">
          ...
          <div>
            <h4 className="text-sm font-black uppercase tracking-widest">Fiscal Integrity Report</h4>
            <p className="text-[10px] font-bold text-blue-200 uppercase tracking-tight mt-1">All financial ledgers successfully reconciled with multi-branch synchronization.</p>
          </div>
        </div>
        <button className="px-8 py-3 bg-white text-[#003366] rounded-xl text-[10px] font-black uppercase tracking-widest hover:scale-105 transition-all relative z-10">
          Dismiss Alert
        </button>
      </div>
    </div>
